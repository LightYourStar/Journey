...
public class ExcelToScriptableObject : EditorWindow
{
    ...
    public static string SEARCH_KEY = string.Empty;
    public static string ExcelPath = "Excels";

    public static string ExcelKey = "ExcelsKey";

    //public static bool OnLine = false;
    //public static string OnLineKey = "OnLineKey";
    private static Regex reg_color32 = new Regex(@"^[A-Fa-f0-9]{8}$");
    private static Regex reg_color24 = new Regex(@"^[A-Fa-f0-9]{6}$");

    private static ErrorCollector errorCheck = new ErrorCollector();

    [MenuItem("Excel/Excel2ScriptableObject", false, 11)]
    static void Excel2ScriptableObject2()
    {
    ...
    // [MenuItem("ProjectTools/ExcelCheck",false,51)]
    // [MenuItem("Excel/ExcelCheck")]
    static void ExcelCheck()
    {
        ...
        //{
        //    Global.gApp = new App();
        //    Global.gApp.gResMgr = new ResBundleMgr();
        //    ExcelDataCheck.GenerateAllFiles();
        //    Global.gApp.gGameData = new GameDatas();

        //    Global.gApp.gSystemMgr = new LDSystemMgr();
        //    Global.gApp.gSystemMgr.CreateAllMgr();
        //    ExcelDataCheck.ExcelDataCheckFun();
        //}
    }
    ...
    static bool Process(ExcelToScriptableObjectSetting excel, bool generateCode)
    {
        try
        {
            errorCheck.SetExcel(excel);
            Stream stream = null;
            string excel_name = excel.GetExcelName();
            int lastDot = excel_name.LastIndexOf('.');
            string tempExcelName = excel_name.Substring(0, lastDot) + "_temp" + excel_name.Substring(lastDot, excel_name.Length - lastDot);
            /*try
            {
                File.Copy(excel_name, tempExcelName, true);
                if (File.Exists(tempExcelName))
                {
                    stream = File.OpenRead(tempExcelName);
                }
                else
                {
                    EditorUtility.DisplayDialog("Excel To Failed", tempExcelName, "OK");
                    return false;
                }
            }
            catch
            {
                string msg = string.Format("Fail to open '{0}' because of sharing violation. Perhaps you should close your Excel application first...", excel_name);
            ...
            try
            {
                // 使用 FileStream 以允许共享读取（Excel 如果不完全独占则可以复制）
                using (var src = new FileStream(excel_name, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                using (var dst = new FileStream(tempExcelName, FileMode.Create, FileAccess.Write, FileShare.None))
                {
                    src.CopyTo(dst);
                }
            ...
            {
                string msg = string.Format("Fail to open '{0}' because of sharing violation. Perhaps you should close your Excel application first...\n{1}", excel_name, e.Message);
                EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                if (File.Exists(tempExcelName))
                {
                    File.Delete(tempExcelName);
            ...
            IExcelDataReader reader = excel_name.ToLower().EndsWith(".xls") ? ExcelReaderFactory.CreateBinaryReader(stream) : ExcelReaderFactory.CreateOpenXmlReader(stream);
            ...
            if (data == null)
            {
                string msg = string.Format("Fail to read '{0}'. It seems that it's not a proper xlsx file...", excel_name);
                EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                return false;
            }

            UpdateOriginalFileConfigs(excel_name, data);
            foreach (DataTable table in data.Tables)
            {
                //string className = Path.GetFileNameWithoutExtension(excel_name);
                string className = table.TableName;
                string assetsName = table.TableName;
                if (className.Equals("iap_jp"))
                {
                    className = "iap";
                }
                else if (className.Equals("MallGoods_jp"))
                {
                    className = "MallGoods";
                }
                ...
                {
                    //string msg = string.Format("Invalid excel file '{0}', because the name of the xlsx file should be a class name...", excel_name);
                    //EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                    //return false;
                    continue;
                }
                ...
                {
                    string msg = string.Format("Fail to parse '{0}'. The excel file should contains at least 2 lines that specify the column names and their types...", excel_name);
                    EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                    return false;
                }
                ...
                for (int i = 0, imax = items.Length; i < imax; i++)
                {
                    ...
                    {
                        string msg = string.Format("Fail to parse '{0}' because of invalid field name '{1}'...", excel_name, fieldName);
                        EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                        return false;
                    }
                ...
                {
                    string msg = string.Format("Fail to parse '{0}' because of no appropriate field names...", excel_name);
                    EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                    return false;
                }
                ...
                for (int i = 0, imax = fieldNames.Count; i < imax; i++)
                {
                    ...
                    if (fieldType == eFieldTypes.Unknown)
                    {
                        if (enumDict == null)
                        {
                            string msg = string.Format("Fail to parse '{0}' because of invalid field type '{1}'...", excel_name, typeName);
                            EditorUtility.DisplayDialog("Excel To ScriptableObject", msg, "OK");
                            return false;
                        }
                ...
                object[] commentItems = table.Rows[global_configs.comment_row].ItemArray;
                for (int i = 0; i < fieldTypeNames.Count; i++)
                {
                    string comment = "";
                    if (i < commentItems.Length)
                    {
                        comment = commentItems[i].ToString().Replace("\n", " ");
                    }

                    commentStrings.Add(comment.Replace("\n", " "));
                }

                List<int> indices = new List<int>();
                excel.generate_get_method_if_possible = true;
                if (excel.generate_get_method_if_possible)
                {
                    ...
                    if (fieldTypes[0] == eFieldTypes.Int)
                    {
                        ...
                        for (int i = global_configs.data_from_row, imax = table.Rows.Count; i < imax; i++)
                        {
                            ...
                            if (string.IsNullOrEmpty(idStr) || idStr.StartsWith("#"))
                            {
                                continue;
                            }
                ...
                if (generateCode)
                {
                    ...
                    content.AppendLine();
                    content.AppendLine(string.Format("\t\tpublic static string CfgName = \"{0}\";", className));
                    content.AppendLine();
                    content.AppendLine(string.Format("\t\tpublic static {0} Data{{ get {{ return Global.gApp.gGameData.GetData<{1}>(CfgName); }} }}", className, className));
                    ...
                    if (true)
                    {
                        ...
                        else
                        {
                            ...
                            content.AppendLine(string.Format("{0}\t\t\t}}", indent));
                            content.AppendLine(string.Format("{0}\t\t}}", indent));
                            content.AppendLine(string.Format("{0}\t\tUnityEngine.Debug.LogError(\"{1}表找不到 => \" + {2});", indent, className, idVarName));
                            content.AppendLine(string.Format("{0}\t\treturn null;", indent));
                            content.AppendLine(string.Format("{0}\t}}", indent));

                            content.AppendLine();

                            content.AppendLine(string.Format("{0}\tpublic bool TryGet({1} {2}, out {3}Item item, bool logError = true) {{", indent, GetFieldTypeName(fieldTypes[0]), idVarName,
                                className));
                            content.AppendLine(string.Format("{0}\t\tint min = 0;", indent));
                            content.AppendLine(string.Format("{0}\t\tint max = items.Length;", indent));
                            content.AppendLine(string.Format("{0}\t\twhile (min < max) {{", indent));
                            content.AppendLine(string.Format("{0}\t\t\tint index = (min + max) >> 1;", indent));
                            content.AppendLine(string.Format("{0}\t\t\titem = _Items[index];", indent));
                            content.AppendLine(string.Format("{0}\t\t\tif (item.{1} == {2}) {{ return true; }}", indent, fieldNames[0], idVarName));
                            if (fieldTypes[0] == eFieldTypes.String)
                            {
                                content.AppendLine(string.Format("{0}\t\t\tif (string.Compare({1}, item.{2}) < 0) {{", indent, idVarName, fieldNames[0]));
                            }
                            ...
                            content.AppendLine(string.Format("{0}\t\t\t}}", indent));
                            content.AppendLine(string.Format("{0}\t\t}}", indent));
                            content.AppendLine(string.Format("{0}\t\titem = null;", indent));
                            content.AppendLine(string.Format("{0}\t\tif (logError) {{ UnityEngine.Debug.LogError(\"{1}表找不到 => \" + {2}); }}", indent, className, idVarName));
                            content.AppendLine(string.Format("{0}\t\treturn false;", indent));
                            content.AppendLine(string.Format("{0}\t}}", indent));
                        }
                    ...
                    for (int i = 0, imax = fieldNames.Count; i < imax; i++)
                    {
                        ...
                        if (excel.use_hash_string && fieldType == eFieldTypes.String)
                        {
                            ...
                            content.AppendLine(string.Format("{0}\t/// <summary>", indent));
                            content.AppendLine(string.Format("{0}\t/// {1}", indent, commentStrings[i]));
                            content.AppendLine(string.Format("{0}\t/// </summary>", indent));
                    ...
                    if (excel.generate_tostring_method)
                    {
                        ...
                        for (int i = 0, imax = fieldNames.Count; i < imax; i++)
                        {
                            ...
                            bool isArray = fieldType == eFieldTypes.Floats || fieldType == eFieldTypes.Ints || fieldType == eFieldTypes.Strings || fieldType == eFieldTypes.Doubles;
                            if (isArray)
                            {
                        ...
                        //数据表对象隐式转换bool符号
                        content.AppendLine(string.Format("{0}\tpublic static implicit operator bool({1}Item item) {{", indent, className));
                        content.AppendLine(string.Format("{0}\t\treturn item != null;", indent));
                        content.AppendLine(string.Format("{0}\t}}", indent));
                        content.AppendLine();
                    }
                ...
                else
                {
                    ...
                    if (excel.script_directory.EndsWith("/"))
                    {
                        assetPath = string.Concat(excel.asset_directory, assetsName, ".asset");
                    }
                    else
                    {
                        assetPath = string.Concat(excel.asset_directory, "/", assetsName, ".asset");
                    ...
                    HashSet<string> NullStr = new HashSet<string>()
                    {
                        "null", "NULL", "Null"
                    };
                    int splitCount = 1000;
                    int index = Mathf.CeilToInt(1.0f * indices.Count / splitCount);
                    for (int m = 0; m < index; m++)
                    {
                        ScriptableObject obj = null; // AssetDatabase.LoadAssetAtPath(assetPath, typeof(ScriptableObject)) as ScriptableObject;

                        string writePath = assetPath;
                        if (m > 0)
                        {
                            if (!Directory.Exists(Application.dataPath + "/ExcelTemp"))
                            {
                                Directory.CreateDirectory(Application.dataPath + "/ExcelTemp");
                            }

                            writePath = string.Concat("Assets/ExcelTemp", "/", assetsName, m.ToString(), ".asset");
                        }
                        ...
                        int imax = Mathf.Min(indices.Count, (m + 1) * splitCount);
                        for (int i = m * splitCount; i < imax; i++)
                        {
                            //     }
                            // }
                            ...
                            for (int j = 0, jmax = fieldNames.Count; j < jmax; j++)
                            {
                                ...
                                switch (fieldTypes[j])
                                {
                                    ...
                                    case eFieldTypes.String:
                                        ...
                                        {
                                            int stringIndex;
                                            if (!hashStrings.TryGetValue(newVal, out stringIndex))
                                            {
                                                stringIndex = hashStrings.Count;
                                                hashStrings.Add(newVal, stringIndex);
                                            }
                                        ...
                                        {
                                            pField.stringValue = newVal;
                                        }

                                        break;
                                    case eFieldTypes.Strings:
                                        string newVals = value;
                                        if (NullStr.Contains(value))
                                        {
                                            newVals = string.Empty;
                                        }

                                        string[] strs = GetStringsFromString(newVals);
                                        pField.ClearArray();
                                        if (excel.use_hash_string)
                                        {
                                        ...
                                        else
                                        {
                                            for (int k = strs.Length - 1; k >= 0; k--)
                                            {
                                                ...
                                                pField.GetArrayElementAtIndex(0).stringValue = str;
                                            }
                                        }
                                    ...
                                    case eFieldTypes.Longs:
                                        long[] longs = GetLongsFromString(value);
                                        pField.ClearArray();
                                        for (int k = longs.Length - 1; k >= 0; k--)
                                        {
                                            pField.InsertArrayElementAtIndex(0);
                                            pField.GetArrayElementAtIndex(0).longValue = longs[k];
                                        }
                        ...
                        AssetDatabase.CreateAsset(obj, writePath);
                    }

                    if (index > 1)
                    {
                        Debug.Log($"文件合并开始 {assetPath}");
                        string parentPath = assetPath;
                        string[] parents = File.ReadAllLines(parentPath);

                        string[] headLines = new string[15];
                        Array.Copy(parents, 0, headLines, 0, 15);
                        List<string> mergedContent = new List<string>(headLines); // 将第一个文件的内容保存到列表中

                        for (int i = index - 1; i >= 1; i--)
                        {
                            var path = string.Concat("Assets/ExcelTemp", "/", assetsName, i.ToString(), ".asset");

                            // 读取文件并去掉前15行
                            string[] fileLines = File.ReadAllLines(path);
                            string[] fileContent = GetContentAfterSkippingLines(fileLines, 15);

                            // 将处理后的文件内容合并到第一个文件
                            mergedContent.AddRange(fileContent);
                        }

                        string[] fileParentContent = GetContentAfterSkippingLines(parents, 15);
                        // 将处理后的文件内容合并到第一个文件
                        mergedContent.AddRange(fileParentContent);

                        // 将合并后的内容写入到新的文件
                        File.WriteAllLines(assetPath, mergedContent);
                        Debug.Log("文件合并成功！");
                        // 删除临时文件
                        Directory.Delete(Application.dataPath + "/ExcelTemp", true);
                        if (File.Exists(Application.dataPath + "/ExcelTemp.meta"))
                        {
                            File.Delete(Application.dataPath + "/ExcelTemp.meta");
                        }
                    }
                }
            }
...
// 跳过文件的前n行并返回剩余内容
    private static string[] GetContentAfterSkippingLines(string[] lines, int skipCount)
    {
        int linesToProcess = lines.Length - skipCount;
        if (linesToProcess <= 0)
        {
            return new string[0]; // 如果跳过的行数大于文件行数，返回空数组
        }

        string[] content = new string[linesToProcess];
        Array.Copy(lines, skipCount, content, 0, linesToProcess);
        return content;
    }
    ...
    private static int[] GetIntsFromString(string str)
    {
        ...
        errorCheck.VerifyArrayContent(str);
    ...
    static void UpdateOriginalFileConfigs(string excelName, DataSet dataSet)
    {
        ...
        if (excel_settings != null)
        {
            foreach (ExcelToScriptableObjectSetting setting in excel_settings)
            {
                if (Path.GetFileNameWithoutExtension(setting.excel_name).Equals(fileName))
                {
                    setting.child_excel_names = excelNames;
                    break;
                }
            }
        }
    ...
    static void ReadsSettings()
    {
        ...
        {
            global_configs = new ExcelToScriptableObjectGlobalConfigs();
        }
    }
    ...
    static ExcelToScriptableObjectSetting ReadExcelSettings(string excelName)
    {
        ...
        for (int i = 0, imax = excel_settings.Count; i < imax; i++)
        {
            ...
            if (excel.GetExcelName() == excelName)
            {
                return excel;
            }
        }
    ...
    private int WriteAssetsWaitFrame = 30;

    void Update()
    {
        if (mToWriteAssets && !EditorApplication.isCompiling)
        {
            WriteAssetsWaitFrame--;
            if (WriteAssetsWaitFrame == 0)
            {
                WriteAssetsWaitFrame = 30;
                ...
                for (int i = 0, imax = excelNames.Length; i < imax; i++)
                {
                    ...
                    if (settings != null)
                    {
                        Process(settings, false);
                    }
                }

                AssetDatabase.Refresh();
                AssetDatabase.SaveAssets();
                errorCheck.ReportErrors();
            }
        }
    }
    ...
    void OnGUI()
    {
        ...
        GUILayout.Space(12f);
        EditorGUILayout.BeginVertical();
        SEARCH_KEY = EditorGUILayout.TextField("Search Key(emepty based)", SEARCH_KEY);
        string excelPath = PlayerPrefs.GetString(ExcelKey);

        ExcelPath = EditorGUILayout.TextField("ExcelPath", excelPath);

        if (ExcelPath != excelPath)
        {
            PlayerPrefs.SetString(ExcelKey, ExcelPath);
            PlayerPrefs.Save();
        }

        //bool OnLineTemp = PlayerPrefs.GetInt(OnLineKey) == 1;
        //OnLine = OnLineTemp;
        //OnLine = EditorGUILayout.Toggle("使用线上表", OnLine);
        //if (OnLineTemp != OnLine)
        //{
        //    PlayerPrefs.SetInt(OnLineKey, OnLine ? 1 : 0);
        //    PlayerPrefs.Save();

        //}
        global_configs.comment_row = EditorGUILayout.IntField("Comment Row (0 based)", global_configs.comment_row);
        ...
        GUILayout.Space(5);
        if (GUILayout.Button("Process Search", GUILayout.Width(100f), GUILayout.Height(30f)))
        {
            GenSearchedTable();
        }
        ...
        if (SEARCH_KEY == string.Empty || processAll)
        {
            for (int i = 0, imax = excel_settings.Count; i < imax; i++)
            {
                CreateExcelItem(excel_settings[i], processAll, i, ref insertAt, ref removeAt, ref cachedGUIBGColor);
            }
        }
        else
        {
            for (int i = 0, imax = excel_settings.Count; i < imax; i++)
            {
                ExcelToScriptableObjectSetting excelToScriptableObjectSetting = excel_settings[i];
                if (Path.GetFileName(excelToScriptableObjectSetting.GetExcelName()).ToLower()
                    .Contains(SEARCH_KEY.ToLower()))
                {
                    CreateExcelItem(excelToScriptableObjectSetting, processAll, i, ref insertAt, ref removeAt,
                        ref cachedGUIBGColor);
                }
                else
                {
                    string excelName = Path.GetFileName(excelToScriptableObjectSetting.GetExcelName())
                        .Replace(".xlsx", "");
                    if (ExcelToScriptableObjectSetting.pinyinDict.ContainsKey(excelName))
                    {
                        string val = ExcelToScriptableObjectSetting.pinyinDict[excelName];
                        if (val.Contains(SEARCH_KEY.ToLower()))
                        {
                            CreateExcelItem(excelToScriptableObjectSetting, processAll, i, ref insertAt, ref removeAt,
                                ref cachedGUIBGColor);
                        }
        ...
        if (mCachedSettings.Count > 0)
        {
            ...
            for (int i = 0, imax = mCachedSettings.Count; i < imax; i++)
            {
                ...
                {
                    processingExcels.Add(mCachedSettings[i].GetExcelName());
                }
            }
    ...
    private void CreateExcelItem(ExcelToScriptableObjectSetting excelToScriptableObjectSetting, bool processAll, int i, ref int insertAt, ref int removeAt, ref Color cachedGUIBGColor)
    {
        ...
        cachedGUIBGColor = GUI.backgroundColor;
        EditorGUILayout.LabelField("Excel File", string.IsNullOrEmpty(setting.GetExcelName()) ? "(Not Selected)" : setting.GetExcelName());
        GUI.backgroundColor = Color.green;
        if (GUILayout.Button("Select", GUILayout.Width(64f)))
        {
            if (Event.current.shift && !string.IsNullOrEmpty(setting.GetExcelName()))
            {
                if (Event.current.button == 0)
                {
                    string folder = Path.GetDirectoryName(setting.GetExcelName());
                    EditorUtility.OpenWithDefaultApp(folder);
                }
                else
                {
                    EditorUtility.OpenWithDefaultApp(setting.GetExcelName());
                }
            }
            else
            {
                ...
                if (!string.IsNullOrEmpty(p))
                {
                    ...
                    if (p.StartsWith(projPath))
                    {
                        p = p.Substring(projPath.Length, p.Length - projPath.Length);
                    }
        ...
        if (isDirEditManually != setting.dir_manual_edit)
        {
            ...
            if (!setting.dir_manual_edit)
            {
                setting.UpdateDirIndices(mFolders);
            }
        }

        EditorGUILayout.EndHorizontal();
        GUILayout.Space(4f);
        if (GameEditorTools.DrawHeader("ExcelConfigParam", "ExcelConfigParam_key", false))
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.BeginVertical();
        ...
        GUILayout.Space(8f);
        GUI.backgroundColor = cachedGUIBGColor;
        bool processable = !string.IsNullOrEmpty(setting.GetExcelName()) &&
    ...
    private void QuickGen()
    {
        GUILayout.Space(4f);
        EditorGUILayout.LabelField("Gen Group :");
        EditorGUILayout.BeginHorizontal();
        if (GUILayout.Button("BattleTest", GUILayout.Height(20f)))
        {
            QuickGenGroup("DYPass", "DYWaveCfg", "DYMonster",
                "DYAISkillN", "DYHero", "DYWeaponCfg", "DYBulletCfg", "DYEffectCfg");
        }

        EditorGUILayout.EndHorizontal();
    }

    private void GenSearchedTable()
    {
        for (int i = 0, imax = excel_settings.Count; i < imax; i++)
        {
            ExcelToScriptableObjectSetting excelToScriptableObjectSetting = excel_settings[i];
            string excelFileName = Path.GetFileNameWithoutExtension(excelToScriptableObjectSetting.GetExcelName());
            if (excelToScriptableObjectSetting.GetExcelName().ToLower().Contains(SEARCH_KEY.ToLower()))
            {
                mCachedSettings.Add(excelToScriptableObjectSetting);
                Debug.Log("Gen : " + excelFileName);
            }
        }
    }

    private void QuickGenGroup(params string[] excelNames)
    {
        HashSet<string> excelNameHash = new HashSet<string>();
        foreach (var excelName in excelNames)
        {
            excelNameHash.Add(excelName.ToLower());
        }
        ...
        for (int i = 0, imax = excel_settings.Count; i < imax; i++)
        {
            ExcelToScriptableObjectSetting excelToScriptableObjectSetting = excel_settings[i];
            string excelFileName = Path.GetFileNameWithoutExtension(excelToScriptableObjectSetting.GetExcelName());
            if (excelNameHash.Contains(excelFileName.ToLower()))
            {
                mCachedSettings.Add(excelToScriptableObjectSetting);
                Debug.Log("Quick Gen : " + excelFileName);
            }
        }
    }
...
public class ExcelToScriptableObjectGlobalConfigs
{
    ...
    public int comment_row = 3;
}

[System.Serializable]
public class ExcelToScriptableObjectSetting : IComparable<ExcelToScriptableObjectSetting>
{
    public string GetExcelName()
    {
        string newExcel = excel_name;
        if (string.IsNullOrEmpty(newExcel))
        {
            return string.Empty;
        }

        return GetExcelName(newExcel);
    }

    public string GetExcelName(string newExcelName)
    {
        ...
        string excelRealName = Path.GetFileName(newExcelName);
        string splitKey = "/diy/";
        string[] path = excel_name.Split(splitKey);
        string ExcelPath = "Excel";
        if (!string.IsNullOrEmpty(ExcelToScriptableObject.ExcelPath))
        {
            ExcelPath = ExcelToScriptableObject.ExcelPath;
        }

        string newDic = Application.dataPath + "/_DesignData/";
        string fileRealPath = newDic + ExcelPath + "/" + excelRealName;
        return fileRealPath;
    }


    public string excel_name;
    public string script_directory = "Assets/Scripts/ConfigClass";
    public string asset_directory = "Assets/Resources/Configs";
    public string name_space = "JO";
    ...
    public bool use_public_items_getter = true;
    ...
    public List<string> child_excel_names = new List<string>();

    public static readonly Dictionary<string, string> pinyinDict = new Dictionary<string, string>()
    {
        { "Q_全局参数", "qjp s" },
    };

    [System.NonSerialized] public bool dir_manual_edit;
    [System.NonSerialized] public int script_dir_index;
    [System.NonSerialized] public int asset_dir_index;

    public int CompareTo(ExcelToScriptableObjectSetting other)
    {
        return string.Compare(GetExcelName(), other.GetExcelName());
    }
    ...
    public void UpdateDirIndices(string[] folders)
    {
        ...
        for (int i = 0, imax = folders.Length; i < imax; i++)
        {
            ...
            if (script_directory == folder)
            {
                script_dir_index = i;
            }

            if (asset_directory == folder)
            {
                asset_dir_index = i;
            }
        }
